...
import { fetchTasks, fetchStatuses, updateTaskStatusAPI } from "@/components/useTasks";
...
export default function TaskDashboard() {
  ...
  async function handleStatusChange(taskId: number, newStatusId: number, e: React.MouseEvent) {
    e.stopPropagation(); // Prevent opening task details modal
    
    try {
      setUpdatingStatus(taskId);
      await updateTaskStatusAPI(taskId, newStatusId, userId || undefined);
          ...
          >
            ...
            <div className="mt-3 flex items-center justify-between text-xs">
              <div onClick={(e) => e.stopPropagation()} className="flex-1">
                <select
                  value={statuses.find(s => s.status === t.status)?.id || ""}
                  onChange={(e) => handleStatusChange(t.id, Number(e.target.value), e as any)}
                  disabled={updatingStatus === t.id}
                  className="text-xs border border-gray-300 rounded px-2 py-1 bg-white hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div className="text-gray-500">{t.endDate ? new Date(t.endDate).toLocaleDateString() : "—"}</div>
            </div>
          </div>
        ))}
        {!sortedTasks.length && (
